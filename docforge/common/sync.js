...
function queueChange(type, action, id, data) {
    ...
    try {
        localStorage.setItem('docforge_sync_queue', JSON.stringify(syncQueue));
    } catch (e) {
...
function clearQueue() {
    syncQueue = [];
    localStorage.removeItem('docforge_sync_queue');
}
...
async function forceSync() {
    ...
    if (typeof DocForgeLoading !== 'undefined') {
        DocForgeLoading.showCloudSync({ message: 'Syncing...' });
    }
...
try {
    const persisted = localStorage.getItem('docforge_sync_queue');
    if (persisted) {
