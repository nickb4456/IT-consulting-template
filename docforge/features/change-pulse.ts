...
const PULSE_CONFIG = {
  ...
  // User preference key
  storageKey: 'docforge_pulse_enabled'
};
...
export class ChangePulse {
  ...
  private showNotification(count: number): void {
    ...
    notification.className = 'docforge-pulse-notification';
    notification.innerHTML = `
      <span class="docforge-pulse-icon">✓</span>
      <span class="docforge-pulse-text">${count} ${count === 1 ? 'change' : 'changes'} applied</span>
    `;
    ...
    requestAnimationFrame(() => {
      notification.classList.add('docforge-pulse-visible');
    });
    
    // Remove after animation
    setTimeout(() => {
      notification.classList.remove('docforge-pulse-visible');
...
.docforge-pulse-notification.docforge-pulse-visible {
  opacity: 1;
