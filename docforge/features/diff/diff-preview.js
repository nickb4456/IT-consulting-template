/**
 * DocForge Compare Changes - UI Component
 * 
...
const DiffPreview = (function() {
    ...
    function attachKeyboardShortcuts() {
        document.addEventListener('keydown', (e) => {
            ...
            if ((e.ctrlKey || e.metaKey) && e.key === 'd' && !e.shiftKey) {
                ...
                if (isVisible) {
                    hide();
                } else if (window.DocForge?.getCurrentFields) {
     ...
     * Integrates with DocForge's field system
     */
    function showForCurrentChanges() {
        ...
        // Get current field values from DocForge
        const currentFields = window.DocForge?.getCurrentFields?.() || [];
        const diffResult = DiffEngine.diffAgainstLastSnapshot(currentFields);
        
        show(diffResult, (diff) => {
            // Apply changes callback - DocForge will handle actual document update
            window.DocForge?.applyFields?.(currentFields);
        });
    }
