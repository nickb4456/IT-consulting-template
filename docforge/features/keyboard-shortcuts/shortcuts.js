/**
 * DocForge Keyboard Ninja Mode - Handler and Registry
 * 
...
const KeyboardShortcuts = (function() {
    ...
    const STORAGE_KEY = 'docforge_shortcuts';
    ...
    function executeAction(shortcut, handler) {
        try {
            ...
            document.dispatchEvent(new CustomEvent('docforge:shortcut', {
    ...
    function registerDefaultHandlers() {
        ...
        registerAction('commandPalette', () => {
            if (typeof CommandPalette !== 'undefined') {
                ...
                // Fallback: emit event
                document.dispatchEvent(new CustomEvent('docforge:openCommandPalette'));
            }
        });
        ...
        registerAction('inlineSearch', () => {
            document.dispatchEvent(new CustomEvent('docforge:openSearch'));
        });
        ...
        registerAction('nextField', () => {
            window.DocForge?.nextField?.();
        });
        
        registerAction('prevField', () => {
            window.DocForge?.prevField?.();
        });
        
        registerAction('acceptField', () => {
            window.DocForge?.acceptField?.();
        });
        
        // Quick Select
        for (let i = 1; i <= 9; i++) {
            registerAction(`quickSelect${i}`, () => {
                window.DocForge?.quickSelect?.(i);
            });
        }
        
        // Edit Actions
        registerAction('undo', () => {
            window.DocForge?.undo?.() || document.execCommand('undo');
        });
        
        registerAction('redo', () => {
            window.DocForge?.redo?.() || document.execCommand('redo');
        });
        
        registerAction('save', () => {
            window.DocForge?.save?.();
        });
        ...
        registerAction('closePanel', () => {
            ...
            document.dispatchEvent(new CustomEvent('docforge:closePanel'));
        });
    }
