...
const DocForgePartyCheckerUI = (function() {
    'use strict';
    ...
    async function handleScan() {
        ...
        try {
            scanResults = await DocForgePartyChecker.scanDocument();
    ...
    async function handleGotoDefinition(partyId) {
        ...
        if (party && party.definitionLocation) {
            try {
                await DocForgePartyChecker.navigateToLocation(party.definitionLocation);
            } catch (error) {
    ...
    async function handleStandardize(partyId) {
        try {
            const result = await DocForgePartyChecker.standardizeParty(partyId);
    ...
    function handleAddParty() {
        ...
        try {
            DocForgePartyChecker.addPartyManually(fullName, shortForm);
    ...
    function handleAddAlias(partyId) {
        ...
        try {
            DocForgePartyChecker.addAlias(partyId, alias);
    ...
    async function handleGotoIssue(issueId) {
        ...
        if (issue && issue.location) {
            try {
                await DocForgePartyChecker.navigateToLocation(issue.location);
            } catch (error) {
    ...
    async function handleFixIssue(issueId) {
        try {
            await DocForgePartyChecker.fixIssue(issueId);
    ...
    async function handleHighlight() {
        try {
            const result = await DocForgePartyChecker.highlightIssues();
    ...
    async function handleFixAll() {
        ...
        for (const issue of fixableIssues) {
            try {
                await DocForgePartyChecker.fixIssue(issue.id);
                fixed++;
            } catch (e) {
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = DocForgePartyCheckerUI;
}
