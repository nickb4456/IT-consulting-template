...
/* global Office, Word, DocForgeStorage, DocForgeUtils */
...
async function quickFill(event) {
    try {
        // Get recent datasets
        const recentDatasets = DocForgeStorage.datasets.getRecent(1);
        ...
        if (recentDatasets.length === 0) {
            ...
            Office.context.mailbox?.item?.notificationMessages?.addAsync('quickfill', {
                ...
                message: 'No recent client profile found. Open DocForge panel to select one.',
                persistent: false
            });
            event.completed();
            return;
...
async function applyNumbering(event) {
    try {
        const state = DocForgeStorage.numbering.getState();
        ...
        let preset = DocForgeStorage.numbering.getPersonalSet(presetId);
        ...
        // Update last used
        DocForgeStorage.numbering.updateState({ lastUsed: presetId });
...
async function applyNumberingPreset(presetId, event) {
    try {
        ...
        // Update last used
        if (typeof DocForgeStorage !== 'undefined') {
            DocForgeStorage.numbering.updateState({ lastUsed: presetId });
        }
...
async function insertHeader(event) {
    try {
        const firmConfig = DocForgeStorage.letterhead.getFirmConfig();
        const currentUser = DocForgeStorage.letterhead.getCurrentUser();
...
async function insertTemplateField(event) {
    try {
        await Word.run(async (context) => {
            ...
            cc.title = DocForgeUtils.toTitleCase(fieldName);
