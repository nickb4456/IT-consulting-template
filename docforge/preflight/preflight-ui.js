...
function initPreflightUI() {
    checker = new DocForgePreflight.Checker();
...
async function runPreflightChecks() {
    ...
    if (typeof DocForgeLoading !== 'undefined') {
        DocForgeLoading.showCheckingAnimation({ 
    ...
    try {
        ...
        const updateProgress = () => {
            if (typeof DocForgeLoading !== 'undefined' && checkIndex < totalChecks) {
                DocForgeLoading.updateCheckingProgress(
                    checkIndex + 1, 
                    totalChecks, 
        ...
        if (typeof DocForgeLoading !== 'undefined') {
            DocForgeLoading.updateCheckingProgress(totalChecks, totalChecks, 'Complete!');
            await new Promise(r => setTimeout(r, 300)); // Brief pause for animation
            DocForgeLoading.hideCheckingAnimation({ 
        ...
        if (typeof DocForgeLoading !== 'undefined') {
            DocForgeLoading.hideCheckingAnimation({ success: false });
        }
