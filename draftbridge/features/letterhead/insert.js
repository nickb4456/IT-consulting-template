/**
 * DraftBridge Letterhead Insertion Module
 * 
...
async function loadFirmConfig() {
    ...
    const stored = localStorage.getItem('draftbridge_firm_config');
    if (stored) {
...
async function saveFirmConfig(config) {
    ...
    localStorage.setItem('draftbridge_firm_config', JSON.stringify(config));
...
async function loadAttorneyProfiles() {
    const stored = localStorage.getItem('draftbridge_attorneys');
    if (stored) {
        return JSON.parse(stored);
    }
    return [];
...
async function saveAttorneyProfile(profile) {
    ...
    localStorage.setItem('draftbridge_attorneys', JSON.stringify(profiles));
}
