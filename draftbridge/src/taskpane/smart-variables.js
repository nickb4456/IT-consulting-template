...
const SmartVariables = {
          ...
          ${builtInTemplates.map(t => `
            <div class="sv-template-card" onclick="SmartVariables.selectTemplate('${t.id}', '${containerId}')">
              ...
              <div class="sv-template-actions">
                <button class="sv-mini-btn" title="Export" onclick="event.stopPropagation(); SmartVariables.exportTemplate('${t.id}')">📤</button>
            ...
            <div class="sv-template-grid">
              ${customTemplates.map(t => `
                <div class="sv-template-card custom" onclick="SmartVariables.selectTemplate('${t.id}', '${containerId}')">
                  ...
                  <div class="sv-template-actions">
                    <button class="sv-mini-btn" title="Export" onclick="event.stopPropagation(); SmartVariables.exportTemplate('${t.id}')">📤</button>
                    <button class="sv-mini-btn danger" title="Delete" onclick="event.stopPropagation(); SmartVariables.deleteTemplate('${t.id}', '${containerId}')">🗑️</button>
                    <span class="sv-template-arrow">→</span>
                  </div>
                </div>
              `).join('')}
    ...
    container.innerHTML = `
      <div class="sv-profile-editor">
        <div class="sv-form-header">
          ...
          <h3 class="sv-template-title">⚙️ My Profile</h3>
        </div>
        ...
        <div class="sv-modal-body" id="sv-contact-picker-list">
          ${recentContacts.length > 0 ? `
            <div class="sv-picker-section">
              <div class="sv-picker-label">⏱️ Recent</div>
  ...
  renderContactPickerItem(contact) {
    ...
    return `
      <div class="sv-picker-item" onclick="SmartVariables.selectContact('${contact.id}')">
        <span class="sv-picker-icon">${contact.isEntity ? '🏢' : '👤'}</span>
      ...
      <div class="sv-form">
        ...
        ${!isValid ? `
          <div class="sv-validation-warning">
            ⚠️ ${Object.keys(this.state.errors).length} required field(s) need attention
          </div>
        ` : ''}
        ...
        if (variable.id === 'court' && this.courts.length > 0) {
          ...
          return `
            <div class="sv-court-select-wrapper">
              ...
              ${value ? `<a class="sv-court-link" href="${this.getCourtById(value)?.filingUrl || '#'}" target="_blank" title="Open ECF Filing">🔗 ECF</a>` : ''}
            </div>
          `;
        }
        ...
        ${parties.map((party, idx) => `
          <div class="sv-party-card ${allowedRoles[0]}">
            <div class="sv-party-header">
              <span class="sv-party-icon">${party.isEntity ? '🏢' : '👤'}</span>
              ...
              <div class="sv-party-actions">
                <button class="sv-mini-btn" title="Save to Contacts" onclick="SmartVariables.addContactFromParty('${variable.id}', ${idx}, '${containerId}')">💾</button>
          ...
          ${recentContacts.length > 0 && !searchQuery ? `
            <div class="sv-contacts-section">
              <div class="sv-contacts-section-header">
                <span class="sv-section-icon">⏱️</span>
  ...
  renderContactCard(contact, isRecent = false) {
    ...
    const subtitle = contact.isEntity ? 
      ...
      [contact.company, contact.email].filter(Boolean).join(' • ');
    const initials = contact.isEntity ? '🏢' : 
    ...
    return `
      <div class="sv-contact-card" data-contact-id="${contact.id}">
        ...
        <div class="sv-contact-actions">
          <button class="sv-icon-btn" title="Edit" onclick="SmartVariables.showEditContactForm('${contact.id}')">✏️</button>
          <button class="sv-icon-btn sv-danger" title="Delete" onclick="SmartVariables.confirmDeleteContact('${contact.id}')">🗑️</button>
        </div>
      </div>
    `;
  },
  ...
  showContactForm(contact) {
    ...
    const formHtml = `
      <div class="sv-modal sv-contact-form-modal">
        <div class="sv-modal-header">
          <h3>${isEdit ? '✏️ Edit Contact' : '➕ Add Contact'}</h3>
