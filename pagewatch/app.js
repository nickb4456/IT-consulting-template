...
async function showAccountView() {
  ...
  try {
    ...
    $('#account-plan').textContent = (data.plan ?? 'free').charAt(0).toUpperCase() + (data.plan ?? 'free').slice(1);
    $('#account-limit').textContent = data.monitorLimit ?? 5;
    maskApiKey();

    // Toggle billing section
    const upgradeEl = $('#billing-upgrade');
    const manageEl = $('#billing-manage');
    if (upgradeEl && manageEl) {
