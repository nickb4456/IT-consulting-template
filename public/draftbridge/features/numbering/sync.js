/**
 * DraftBridge Numbering Set Sync Module
 * 
...
// Storage keys
const STORAGE_KEYS = {
    PERSONAL_SETS: 'draftbridge_numbering_personal',
    FIRM_CACHE: 'draftbridge_numbering_firm_cache',
    FIRM_CACHE_ETAG: 'draftbridge_numbering_firm_etag',
    SYNC_QUEUE: 'draftbridge_numbering_sync_queue',
    USER_STATE: 'draftbridge_numbering_state' // Last used, default, recent
};
...
class NumberingSetManager {
    ...
    async exportSet(id) {
        ...
        return {
            format: 'draftbridge-numbering-v1',
    ...
    importSet(jsonData, targetScope = 'personal') {
        ...
        if (jsonData.format !== 'draftbridge-numbering-v1') {
            throw new Error('Invalid format: Expected draftbridge-numbering-v1');
        }
    ...
    async downloadSetAsFile(id) {
        ...
        const set = await this.getSet(id);
        const filename = `${set.name.replace(/[^a-z0-9]/gi, '-').toLowerCase()}.draftbridge-numbering.json`;
