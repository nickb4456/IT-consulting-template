...
<script>
  ...
  Office.onReady(() => {
    DraftBridgePartyCheckerUI.init('party-checker-panel');
  });
</script>
```
...
const registry = DraftBridgePartyChecker.getRegistry();

// Standardize a party's references
await DraftBridgePartyChecker.standardizeParty('party_1');

// Highlight all issues in document
await DraftBridgePartyChecker.highlightIssues();

// Clear highlights
await DraftBridgePartyChecker.clearHighlights();

// Navigate to a location
await DraftBridgePartyChecker.navigateToLocation({ paragraph: 5, offset: 0 });

// Add a party manually
DraftBridgePartyChecker.addPartyManually('Acme Corporation', 'Acme', ['ACME', 'Acme Corp']);

// Fix a specific issue
await DraftBridgePartyChecker.fixIssue('issue_123');
```
...
console.log(DraftBridgePartyChecker.COMMON_SHORT_FORMS);
console.log(DraftBridgePartyChecker.ENTITY_TYPES);
```
